...
--
-- Iterate over each language configuration in M.languages
--for _, config in pairs(langs.languages) do
    -- Check if the language has an LSP configuration
  --  if config.lsp then
        -- Dynamically generate the setup call using the language's LSP name
    --    local lsp_setup_call = string.format("require('lspconfig').%s.setup {}", config.lsp)

        -- Execute the setup call
      --  pcall(lsp_setup_call)
   -- end
----end
